...
TEST(ResultPracticalTest, DivideExample)
{
    auto r1 = divide(10, 2);
    EXPECT_TRUE(r1.is_ok());
    EXPECT_EQ(r1.unwrap(), 5);

    auto r2 = divide(10, 0);
    EXPECT_TRUE(r2.is_err());
    EXPECT_EQ(r2.unwrap_err(), "Division by zero");
}
...
TEST(ResultPracticalTest, ValidationExample)
{
    auto r1 = validate_age(25);
    EXPECT_TRUE(r1.is_ok());

    auto r2 = validate_age(-5);
    EXPECT_TRUE(r2.is_err());
    EXPECT_EQ(r2.unwrap_err(), "Age cannot be negative");

    auto r3 = validate_age(200);
    EXPECT_TRUE(r3.is_err());
    EXPECT_EQ(r3.unwrap_err(), "Age too high");
}
...
TEST(ResultPracticalTest, ParseExample)
{
    auto r1 = parse_number("42");
    EXPECT_TRUE(r1.is_ok());
    EXPECT_EQ(r1.unwrap(), 42);

    auto r2 = parse_number("not a number");
    EXPECT_TRUE(r2.is_err());
}
